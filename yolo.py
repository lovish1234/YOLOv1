...
class Yolo:
    ...
    def __init__(self):
        ...
        if self.mode=='testDB':    
            for fileName in os.listdir(self.inputFolder): 
                ...
                annotatedImage,predictedObjects = self.detect_from_file(inputFile)
                ...
                if self.saveAnnotatedXML:
                    outputTextFile = os.path.join(self.textOutputFolder,fileName)
                    root = ET.Element("annotation")
                    ET.SubElement(root,"filename").text = fileName
                    for i in range(len(predictedObjects)):
                        object = ET.SubElement(root,"object")            
                        ET.SubElement(object,"name").text = predictedObjects[i][0]
                        ET.SubElement(object,"confidence").text = str(predictedObjects[i][5])
                        bndbox = ET.SubElement(object,"bndbox")
                        ET.SubElement(bndbox,"xmin").text = str(predictedObjects[i][1])
                        ET.SubElement(bndbox,"ymin").text = str(predictedObjects[i][2])
                        ET.SubElement(bndbox,"xmax").text = str(predictedObjects[i][3])
                        ET.SubElement(bndbox,"ymax").text = str(predictedObjects[i][4])
            ...
            while(True):
                # capture frames
                ret, frame = cap.read()
                annotatedImage,predictedObjects = self.detect_from_matrix(frame)
    ...
    def calculate_loss_function(self,predicted,groundTruth):
        ...
        predictedParameters = np.reshape(predicted,[-1,self.numOfGrids,self.numOfGrids,30])
        ...
        lossFirstBoxes = tf.reduce_sum(tf.square(predictedFirstBoxes-groundTruthBoxes),3)
        lossSecondBoxes = tf.reduce_sum(tf.square(predictedSecondBoxes-groundTruthBoxes),3)
        
        # getting which box ( bbox1 or bbox2 ) is responsible for the detection  
        IOU = iouTrain(predictedFirstBoxes,predictedSecondBoxes,groundTruthBoxes)
        responsbileBox = tf.greater(IOU[:,:,:,0],IOU[:,:,:,1])
        # suppose it is known that which iou is greater

        # coordinate loss ( loss due to difference in coordinates of predicted-responsible and real box )
        coordinateLoss = tf.where(responsibleBox,lossFirstBoxes,lossSecondBoxes)
        ...
        # gives a sparse -1x7x7x1 matrix, only one element would be nonzero in each slice
        coorinateLoss = self.lambdaCoordinate*tf.multiply(groundTruthGrid,coordinateLoss)

        # object loss ( loss due to difference in object confidence )
        # only take the objectLoss of the predicted grid with higher IoU is responsible for the object
        ...
        classLoss = tf.reduce_sum(tf.mul(groundTruthGrid,classLoss),reduction_indices=3)
        classLoss = tf.reshape(classLoss,[-1,7,7,1])
        
        # no-object loss, decrease the confidence where there is no object in the ground truth
        noObjectLoss = self.lambdaNoObject*tf.multiply(1-groundTruthGrid,tempObjectLoss) 
        # total loss
        totalLoss = coordinateLoss + objectLoss + classLoss + noObjectLoss
        totalLoss = tf.reduce_mean(tf.reduce_sum(totalLoss, reduction_indeces=[1,2,3]),reduction_indices=0)
    ...
    def conv_layer(self,index,inputMatrix,numberOfFilters,sizeOfFilter,stride):
        ...
        weight =tf.Variable(tf.truncated_normal([sizeOfFilter,sizeOfFilter,int(numberOfChannels),numberOfFilters],stddev=0.1))
        ...
        paddedInput=tf.pad(inputMatrix,([[0,0],[padSize,padSize],[padSize,padSize],[0,0]]))

        conv = tf.nn.conv2d(paddedInput,weight,strides=[1,stride,stride,1],padding='VALID',name=str(index)+'_conv')
    ...
    def maxpool_layer(self,index,inputMatrix,sizeOfFilter,stride):
        ...
        maxpool = tf.nn.max_pool(inputMatrix,ksize=[1,sizeOfFilter,sizeOfFilter,1],strides=[1,sizeOfFilter,sizeOfFilter,1],padding='SAME',name=str(index)+'_maxpool')
        return maxpool
    ...
    def fc_layer(self,index,inputMatrix,outputNeurons,flatten,linear):
        ...
        if flatten:
            ...
            inputMatrixAdjust = tf.reshape(inputMatrixAdjust,[-1,inputDimension])
        else:
            inputDimension = inputShape[1]
            inputMatrixAdjust = inputMatrix
        
        weight = tf.Variable(tf.truncated_normal([inputDimension,outputNeurons],stddev=0.1))
        ...
        if linear:
            ...
            answer =  tf.add(tf.matmul(inputMatrixAdjust,weight),bias,name=str(index)+'_fc')
    ...
    def interpret_output(self,netOutput):
        ...
        classProbability = np.zeros([self.numOfGrids,self.numOfGrids,self.numOfBoxes,self.numOfClasses])
        ...
        boxData = np.reshape(netOutput[:,1078:],[self.numOfGrids,self.numOfGrids,self.numOfBoxes,4])
        ...
        # changing x and y coordinates from model representation to image representation 
        boxData[:,:,:,0] = ((boxData[:,:,:,0] + offset[:,:,:])*1.0/self.numOfGrids)*self.imageWidth
        boxData[:,:,:,1] = ((boxData[:,:,:,1] + np.transpose(offset,(1,0,2)))*1.0/self.numOfGrids)*self.imageHeight
        ...
        filteredBoxes = boxData[filterProbabilityIndex[0],filterProbabilityIndex[1],filterProbabilityIndex[2]]
        
        filteredClasses = np.argmax(filterClassProbability,axis=3)[filterProbabilityIndex[0],filterProbabilityIndex[1],filterProbabilityIndex[2]]
        ...
        result = []
        for i in range(len(filteredBoxes)):
            result.append([self.classes[filteredClasses[i]],filteredBoxes[i][0],filteredBoxes[i][1],filteredBoxes[i][2],filteredBoxes[i][3],filteredProbability[i]])
        return result
    ...
    def iouTrainUnit(self,boxA,realBox):
        ...
        intersectionX = tf.minimum(boxA[:,:,:,0]+0.5*boxA[:,:,:,2], realBox[:,:,:,0]+0.5*realBox[:,:,:,2])-tf.maximum(boxA[:,:,:,0]-0.5*boxA[:,:,:,2],realBox[:,:,:,0]-0.5*realBox[:,:,:,2])
        intersectionY = tf.minimum(boxA[:,:,:,1]+0.5*boxA[:,:,:,3], realBox[:,:,:,1]+0.5*realBox[:,:,:,3])-tf.maximum(boxA[:,:,:,1]-0.5*boxA[:,:,:,3],realBox[:,:,:,1]-0.5*realBox[:,:,:,3])
